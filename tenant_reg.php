...
if(isset($_REQUEST['reg'])) //if register button is clicked
{
	...
	$pass= sha1($pass); //encrypting password
	...
	else
	{
		...
		if(!empty($name) && !empty($email) && !empty($phone) && !empty($pass) && !empty($uimage))
		{
			...
			$sql="INSERT INTO user (uname,uemail,uphone,upass,utype,uimage) VALUES ('$name','$email','$phone','$pass','$utype','$uimage')"; //Inserting a new record to db
			$result=mysqli_query($con, $sql);
			move_uploaded_file($temp_name1,"Landlord/user/$uimage"); //Moving the uploaded image to Landlord/user folder
			   if($result){
